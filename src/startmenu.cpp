...
/**
 * @brief Zeichnet das Hintergrundbild, die Menüoptionen und das Symbol im angegebenen Renderfenster.
 *
 * @param window Referenz auf das Renderfenster
 */
void StartMenu::draw(sf::RenderWindow &window)
{
    window.draw(backgroundSprite); // Zeichnet den Hintergrund
    for (int i = 0; i < MAX_NUMBER_OF_ITEMS; i++)
    {
        window.draw(menu[i]); // Zeichnet die Menüoptionen
    }
    window.draw(iconSprite); // Zeichnet das Symbol
}
